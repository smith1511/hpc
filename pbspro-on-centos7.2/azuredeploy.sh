...
PBSPRO_GROUP=pbs
PBSPRO_GID=6006
PBSPRO_VERSION=15-08-1-1
PBSPRO_CONF_DIR=$SHARE_DATA/conf
...
# Downloads, builds and installs PBS Pro OSS on the node.
# Starts the PBS Pro control daemon on the master node and
# the agent on worker nodes.
#
install_pbspro()
{
    groupadd -g $PBSPRO_GID $PBSPRO_GROUP
    useradd -M -u $PBSPRO_UID -c "PBS Pro service account" -g $PBSPRO_GROUP -s /usr/sbin/nologin $PBSPRO_USER
