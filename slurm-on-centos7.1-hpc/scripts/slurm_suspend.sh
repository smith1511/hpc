...
echo "`date` Suspend invoked $0 $*" >> /var/log/slurmctld/power_save.log
...
for host in $hosts
do
   echo "Deallocating node $host" >> /var/log/slurmctld/power_save.log

   azure vm deallocate --resource-group $RESOURCE_GROUP --name $host
